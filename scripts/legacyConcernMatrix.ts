import { IngredientTag } from '../src/services/ingredientInteractions';
...
const productStore = new Map<string, { info: ProductInfo; aliases: Set<string> }>();
...
function registerProduct(
  ...
  const entry = productStore.get(primaryName) ?? {
    info: normalizedInfo,
    aliases: new Set<string>(),
  };
  entry.info = normalizedInfo;
  entry.aliases.add(primaryName);
  for (const alias of aliases) {
    if (!alias) continue;
    entry.aliases.add(alias);
    productRegistry.set(alias.trim().toLowerCase(), normalizedInfo);
  }
  productRegistry.set(primaryName.trim().toLowerCase(), normalizedInfo);
  productStore.set(primaryName, entry);
}
...
export const legacyProducts = Array.from(productStore.entries()).map(([name, data]) => ({
  name,
  info: data.info,
  aliases: Array.from(
    new Set(Array.from(data.aliases).map(alias => alias.trim()).filter(Boolean))
  ),
}));

export const legacySkinTypeDefaults = SKIN_TYPE_DEFAULTS;
export const legacyRawMatrix = RAW_MATRIX;
export const legacyDynamicSunscreens = DYNAMIC_SUNSCREENS;
