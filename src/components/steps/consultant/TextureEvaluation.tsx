...
const TextureEvaluation: React.FC<StepProps> = ({ formData, updateFormData, onNext, onBack, errors }) => {
  ...
  return (
    <FormStep
      title={<span className="text-3xl font-extrabold tracking-tight text-gray-900 drop-shadow font-sans">Texture Evaluation</span>}
      onNext={onNext}
      onBack={onBack}
      isValid={isValid()}
    >
      <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 gap-3">
            {textureOptions.map((option) => (
              <label key={option.id} className={`flex items-center gap-3 p-4 rounded-xl border transition-colors cursor-pointer font-sans
                ${formData.evaluation?.texture_evaluation?.checked?.[0] === option.id
              ...
              `}>
                <input
                  ...
                  checked={formData.evaluation?.texture_evaluation?.checked?.[0] === option.id}
          ...
          <div className="grid grid-cols-2 gap-3">
            {classifications.map((classification) => (
              <label key={classification.id} className={`p-4 rounded-xl border-2 cursor-pointer font-sans text-lg font-semibold transition-colors
                ${formData.evaluation?.texture_evaluation?.classification === classification.id
                  ? 'border-blue-500 bg-blue-50 text-gray-900'
                  : 'border-gray-200 bg-white text-gray-800 hover:border-blue-300 hover:bg-blue-50'}
              `}>
                <input
                  ...
                  checked={formData.evaluation?.texture_evaluation?.classification === classification.id}
        ...
        {errors.texture_evaluation && (
          <p className="text-rose-500 text-sm mt-2 font-sans">{errors.texture_evaluation}</p>
        )}
      </div>
    </FormStep>
  );
